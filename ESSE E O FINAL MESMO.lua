...
local ValidTargetParts = {"Head", "UpperTorso", "LowerTorso", "LeftLowerLeg", "RightLowerLeg"}
...
local function getClosestPlayer()
    ...
    for _, Player in pairs(Players:GetPlayers()) do
        ...
        if SilentAimSettings.AliveCheck then
        ...
        local Target = Character:FindFirstChild(SilentAimSettings.TargetPart)
        if not Target then continue end

        local screenPos, onScreen = WorldToScreen(Camera, Target.Position)
        if not onScreen then continue end

        local dist = (getMousePosition() - Vector2.new(screenPos.X, screenPos.Y)).Magnitude
        if not DistanceToMouse or dist < DistanceToMouse then
...
local Window = Library:CreateWindow({Title = 'Universal Silent Aim', Center = true, AutoShow = false, TabPadding = 8, MenuFadeTime = 0.2})
...
local MainBOX = GeneralTab:AddLeftTabbox("Main") do
    ...
    Main:AddDropdown("TargetPart", {AllowNull = true, Text = "Target Part", Default = SilentAimSettings.TargetPart, Values = {"Head", "HumanoidRootPart", "Random"}}):OnChanged(function()
        SilentAimSettings.TargetPart = SilentAimSettings.TargetPart
    end)
...
local LoadConfigurationBOX = GeneralTab:AddRightTabbox("Load Configuration") do 
    ...
    Main:AddButton("Load Configuration", function()
        if table.find(GetFiles(), Options.LoadConfigurationDropdown.Value) then
           ...
           SilentAimSettings.TargetPart:SetValue(SilentAimSettings.TargetPart)
...
resume(create(function()
    -- Cache target each frame to avoid instability
local CachedTarget = nil
RunService.RenderStepped:Connect(function()
    CachedTarget = getClosestPlayer()

    -- Update FOV circle and box
    if Toggles.MousePosition.Value and SilentAimSettings.Enabled and CachedTarget then
        local RootToViewportPoint, IsOnScreen = WorldToViewportPoint(Camera, CachedTarget.Position)
...
oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, Index)
    if self == Mouse and not checkcaller() and SilentAimSettings.Enabled and Options.Method.Value == "Mouse.Hit/Target" and CachedTarget then
        local HitPart = CachedTarget
         
        if Index == "Target" or Index == "target" then 
            return CachedTarget
        elseif Index == "Hit" or Index == "hit" then 
            return ((Toggles.Prediction.Value and (CachedTarget.CFrame + (CachedTarget.Velocity * PredictionAmount))) or (not Toggles.Prediction.Value and CachedTarget.CFrame))
