...
local function getClosestPlayer()
    ...
    local radius = getgenv().SilentAimSettings.FOVRadius or 2000
    local DistanceToMouse = radius + 1 -- começa maior que o raio para filtrar direito
    local Closest = nil
    ...
    for _, Player in next, GetPlayers(Players) do
        ...
        if Distance <= radius and Distance < DistanceToMouse then
...
local Window = Library:CreateWindow({Title = 'Universal Silent Aim', Center = true, AutoShow = false, TabPadding = 8, MenuFadeTime = 0.2, ToggleKey = Enum.KeyCode.Unknown})
...
resume(create(function()
    RenderStepped:Connect(function()
        if Toggles.MousePosition.Value and SilentAimSettings.Enabled then
        ...
        if getgenv().SilentAimSettings.FOVVisible then
